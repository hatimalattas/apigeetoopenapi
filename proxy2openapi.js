...
async function genopenapi(location, answers, xmlFile, cb) {
  var badRequestErrors = [{ "code": "400", "message": "Bad Request" }];
  var unauthorizedRequestErrors = [{ "code": "401", "message": "Unauthorized" }];
  var forbiddenRequestErrors = [{ "code": "403", "message": "Forbidden" }];
  var pageNotFoundErrors = [{ "code": "404", "message": "Not Found" }];
  var internalServerErrors = [{ "code": "500", "message": "Internal Server Error" }];
  var openapiJson = {};
  var reply = await loadXMLDoc(location + "/apiproxy/" + answers.api + ".xml");
  ...
  var replyProxy = await loadXMLDoc(xmlFile);
  // Add base path
  try {
    var basePath = replyProxy.ProxyEndpoint.HTTPProxyConnection[0].BasePath[0];
    servers.push({ "url": protocol.substring(0, protocol.length - 1) + "://" + proxy.host + basePath })
  } catch (ex) { console.log(ex); }
    ...
    if (openapiPath["Condition"] != null) {
      ...
      if (verbArr != null && pathArr != null) {
        ...
        for (stepKey in openapiPath.Request[0].Step) {
          ...
          var replyStep = await loadXMLDoc(location + "/apiproxy/policies/" + flowStepPath.Name + ".xml");
          ...
          if (replyStep.ExtractVariables) {
            ...
            if (source == 'request') {
              try {
                ...
                addParametersAndRequestBody(replyStep.ExtractVariables.Header, 'header', openapiJson.paths[resourcePath][resourceVerb], replyStep.ExtractVariables["IgnoreUnresolvedVariables"][0])
              } catch (error) {
              }
              try {
                // Capture QueryParam
                addParametersAndRequestBody(replyStep.ExtractVariables.QueryParam, 'query', openapiJson.paths[resourcePath][resourceVerb], replyStep.ExtractVariables["IgnoreUnresolvedVariables"][0])
              } catch (error) {
              }
              try {
                // Capture FormParam
                addParametersAndRequestBody(replyStep.ExtractVariables.FormParam, 'formData', openapiJson.paths[resourcePath][resourceVerb], replyStep.ExtractVariables["IgnoreUnresolvedVariables"][0])
              } catch (error) {
              }
              try {
                // Capture JSON Body
                addParametersAndRequestBody(replyStep.ExtractVariables.JSONPayload[0]["Variable"], 'requestBody', openapiJson.paths[resourcePath][resourceVerb], replyStep.ExtractVariables["IgnoreUnresolvedVariables"][0])
              } catch (error) {
              }
            }
          }
          if (replyStep.RaiseFault) {
            try {
              if (replyStep.RaiseFault.FaultResponse[0].hasOwnProperty("AssignVariable")) {
                var errorMessage = replyStep.RaiseFault.FaultResponse[0].AssignVariable[0].Value[0];
                var errorCode = replyStep.RaiseFault.FaultResponse[0].AssignVariable[1].Value[0];
              } else {
                var errorMessage = JSON.parse(replyStep.RaiseFault.FaultResponse[0].Set[0]["Payload"][0]["_"])["error_message"];
                var errorCode = replyStep.RaiseFault.FaultResponse[0].Set[0]["StatusCode"][0];
              }
              if (errorCode == "400") {
                let errorPayload = {
                  "code": errorCode,
        ...
        for (stepKey in openapiPath.Response[0].Step) {
          var flowStepPath = JSON.parse(JSON.stringify(openapiPath.Response[0].Step[stepKey]));
          if (flowStepPath.Name[0].includes("OAS")) {
            ...
            if (replyStep.AssignMessage) {
              ...
              if (source == 'response') {
                try {
                  var payload = replyStep.AssignMessage.Set[0]["Payload"][0]["_"];
                  var contentType = replyStep.AssignMessage.Set[0]["Payload"][0]["$"]["contentType"];
                  if (replyStep.AssignMessage.Set[0].hasOwnProperty("StatusCode")) {
                    var httpStatusCode = replyStep.AssignMessage.Set[0]["StatusCode"][0];
  ...
  function addParametersAndRequestBody(paramArr, openapiType, openapiPath, ignoreUnresolvedVariables) {
    var isRequired = ignoreUnresolvedVariables !== "true";
    ...
    if (openapiType === 'requestBody') {
      ...
      var requestBodySchema = openapiPath.requestBody.content["application/json"].schema;

      paramArr.forEach(param => {
        var paramName = param.$.name;
        var paramType = param.$.type !== undefined ? param.$.type : "string";
        paramType = paramType === "float" ? "number" : paramType;
        var propertyDescription = param.$.description !== undefined ? param.$.description : "";
        var propertyPlaceholder = param.$.placeholder !== undefined ? param.$.placeholder : "";
      ...
      paramArr.forEach(param => {
        ...
        var paramType = param.$.type !== undefined ? param.$.type : "string";
        paramType = paramType === "float" ? "number" : paramType;
        var paramDescription = param.$.description !== undefined ? param.$.description : "";
        var paramPlaceholder = param.$.placeholder !== undefined ? param.$.placeholder : "";
  ...
  function createErrorResponse(errorList) {
    ...
    if (errorList.length === 1) {
      ...
      response.content["application/json"].schema.oneOf = errorList.map((error, index) => ({
        "type": "object",
        "properties": {
          "code": {
            "type": "string",
...
function loadXMLDoc(filePath) {
  return new Promise((resolve, reject) => {
    try {
      var fileData = fs.readFileSync(filePath, 'utf-8'); // Change 'ascii' to 'utf-8' to read Arabic characters
      var parser = new xml2js.Parser();
      parser.parseString(fileData, (err, result) => { // No need for substring
        if (err) {
          reject(err);
        } else {
          resolve(result);
